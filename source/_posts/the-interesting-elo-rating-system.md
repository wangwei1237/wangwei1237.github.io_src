...
根据实践，我们不能采用平均分的方式对两个编解码器效果进行评估，否则可能会存在偏差。以如上的打分为例，其平均打分为：$\mu_{MOS\_Scores}=50$，但是我们不能说两个编解码器的效果是一致的。因为 B 对于 A 而言，其 GSB（Good/Same/Bad）分布为：40%/40%/20%。因此，对于我们的评估系统而言，我们一般用 GSB 的分布，而不是一个单一的分数来确定哪个编解码器更好。
...
可以使用 [elo.py](https://github.com/wangwei1237/wangwei1237.github.io_src/blob/master/source/_posts/the-interesting-elo-rating-system/elo.py) 脚本 [对 100 个评估视频的 MOS 得分](https://github.com/wangwei1237/wangwei1237.github.io_src/blob/master/source/_posts/the-interesting-elo-rating-system/MOS) 进行 Elo 积分迭代，其 Elo 积分的迭代如下图所示：
